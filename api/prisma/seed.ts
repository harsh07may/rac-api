...
async function seedRolesAndPermissions() {
  ...
  // ✅ Role definitions using v2
  const roles = [
    {
      name: "ADMIN",
      permissions: allPermissions, // full access
    },
    {
      name: "AGENCY_MANAGER",
      permissions: [
    ...
    {
      name: "DRIVER",
      permissions: ["read:bookings", "update:bookings"],
    },
    {
      name: "CUSTOMER",
      permissions: ["read:vehicles", "create:bookings", "read:bookings"],
    },
  ];
...
async function seedGeography() {
  ...
  try {
    ...
    if (existingCountries < 50) {
      console.log(
        "Seeding bulk geographical data. This might take a minute...",
      );
      ...
      const statesData = JSON.parse(
        await fs.readFile(
          new URL("./data/states.json", import.meta.url),
          "utf-8",
        ),
      );
      const stateInserts = statesData
        .map((s: any) => ({
          name: s.name,
          countryId: countryMap.get(s.country_code),
        }))
      ...
      const cityInserts = citiesData
        .map((c: any) => ({
          name: c.name,
          countryId: countryMap.get(c.country_code),
        }))
    ...
    console.error(
      "Failed to seed geographic data. Please ensure data/*.json files exist.",
      e.message || e,
    );
  }
}
...
async function seedAddons() {
  ...
  const addons = [
    ...
    },
    {
      name: "GPS Navigation",
      price: 15.0,
      description: "Stand-alone GPS navigation unit",
    },
    {
      name: "Additional Driver",
      price: 50.0,
      description: "Allows an additional driver to operate the vehicle",
    },
    {
      name: "Premium Insurance Cover",
      price: 100.0,
      description: "Zero liability premium insurance cover",
    },
    {
      name: "Roadside Assistance",
      price: 20.0,
      description: "24/7 priority roadside assistance",
    },
  ];
